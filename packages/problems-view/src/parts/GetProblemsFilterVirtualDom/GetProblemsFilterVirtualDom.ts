...
export const getProblemsFilterVirtualDom = (action: ViewletAction): readonly VirtualDomNode[] => {
  return [
    ...
    getInputBoxVirtualDom(action.name || InputName.ProblemsInput, action.command, action.placeholder || '', action.value),
    ...getFilterBadgeDom(action.badgeText),
    ...GetActionButtonVirtualDom.getActionButtonVirtualDom({
      command: DomEventListenerFunctions.HandleClickMoreFilters,
      icon: MaskIcon.Filter,
      id: ProblemStrings.moreFilters(),
      name: 'more filters',
    }),
  ]
}
